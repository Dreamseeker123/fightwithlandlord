		...
		public void run() {
			...
			try {
				...
				if(pc.getGameState()==GameState.WAITCLIENT){
					pc.getPlayers()[index].setReady(socketdata2.isReady());
				}else if(pc.getGameState()==GameState.WAITCALL ){
					pc.getPlayers()[index].setIsCallLandlord(0);
				}else if(pc.getGameState()==GameState.PLAYING){
					pc.getPlayers()[index].setPassCard(socketdata2.isPassCard());
				}
